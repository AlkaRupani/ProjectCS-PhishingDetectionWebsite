...
class FeatureExtraction:
    ...
    def DomainRegLen(self):
        try:
            ...
            crt_date = self.whois_response.creation_date
            if isinstance(exp_date, list): exp_date = exp_date[0]
            if isinstance(crt_date, list): crt_date = crt_date[0]
            age = (exp_date.year - crt_date.year) * 12 + (exp_date.month - crt_date.month)
    ...
    def RequestURL(self):
        try:
            ...
            for tag in ['img', 'audio', 'embed', 'iframe']:
                for elem in self.soup.find_all(tag, src=True):
                    i += 1
                    if self.domain in elem['src']:
                        success += 1
    ...
    def AnchorURL(self):
        try:
            i, unsafe = 0, 0
            for a in self.soup.find_all('a', href=True):
                if "#" in a['href'] or "javascript" in a['href'].lower() or self.domain not in a['href']:
                    unsafe += 1
                i += 1
    ...
    def LinksInScriptTags(self):
        try:
            ...
            for tag in ['link', 'script']:
                for elem in self.soup.find_all(tag, href=True) if tag == 'link' else self.soup.find_all(tag, src=True):
                    i += 1
                    src = elem.get('href') if tag == 'link' else elem.get('src')
    ...
    def IframeRedirection(self):
        try:
            return -1 if re.search("<iframe>|<frameBorder>", self.response.text) else 1
        except:
            return -1
    ...
    def AgeofDomain(self):
        try:
            ...
            age = (date.today().year - creation_date.year) * 12 + (date.today().month - creation_date.month)
    ...
    def PageRank(self):
        try:
            ...
            global_rank = int(re.findall(r"Global Rank: ([0-9]+)", resp.text)[0])
            return 1 if global_rank < 100000 else -1
        except:
            return -1
    ...
    def StatsReport(self):
        try:
            malicious_urls = r'at\.ua|usa\.cc|baltazarpresentes\.com\.br|pe\.hu|esy\.es|hol\.es|sweddy\.com'
            return -1 if re.search(malicious_urls, self.url) else 1
        except:
            return 1
